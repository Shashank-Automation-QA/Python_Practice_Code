...
# get the browser from user and compare with tuple  , if user passes any other ,then it should print such as user is allowed to executed which is available
supported_browsers = ("Chrome", "Firefox", "Edge", "Safari")
User_Browser=input("Enter browser name: ").capitalize()
if User_Browser in supported_browsers:
    print("Valid")
else:
    print("InValid")

#WAP to satisfy below scenario
# " 1. Total test Count, 2. Passed Count, 3. Failed count, 4. Failed test name, 5.slowtest test, 6.avg response time"
test_results = (("TC001", "Login", 120, "PASS"),("TC002", "Search", 450, "PASS"),("TC003", "Payment", 890, "FAIL"),("TC004", "Logout", 150, "PASS"),("TC005", "Profile", 340, "FAIL"))
TC_Count = 0
Pass_Count = 0
Failed_Count = 0
Slowest_test_time = 0
    ...
    for value in tup:
        if isinstance(value, str):
            if value.startswith("TC"):
                ...
                Failed_Count += 1
                Failed_TC_Names.append(tup[0])
        else:
            Time_list.append(int(value))
            if value>Slowest_test_time:
                Slowest_test_time = int(value)

Avg_Response_Time =sum(Time_list)/len(Time_list)
print(f"Total test Count :{TC_Count} , Passed Count :{Pass_Count},  Failed count: {Failed_Count}, Failed test name : {Failed_TC_Names}, slowtest test: {Slowest_test_time}, avg response time: {Avg_Response_Time}")
